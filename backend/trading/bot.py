...
class TradingBot:
    ...
    def load_state_sync(self):
        ...
        try:
            if self.state_file.exists():
                with open(self.state_file, 'r', encoding='utf-8') as f:
                    state = json.load(f)
                    self.last_file_mtime = self.state_file.stat().st_mtime  # 마지막 파일 수정 시간 기록
                    return state
        except Exception:
        ...
        return {  # 로드 실패 시 기본값 반환
            ...
            "target_price": 0, "stop_loss": 0, "balance": 10000, "last_reason": ""
        }
    ...
    async def execute_buy(self, price, reason, trade_params):
        self.state.update({  # 매수 시 상태 업데이트
            "is_holding": True,
            "avg_buy_price": price,
            "target_price": trade_params.get("target_price", price * 1.05),  # 지정 목표가 없을 경우 5% 상향
            "stop_loss": trade_params.get("stop_loss", price * 0.95),  # 지정 손절가 없을 경우 5% 하향
    ...
    async def execute_sell(self, price, reason, event_name="SELL"):
        ...
        profit_pct = ((price - avg_price) / avg_price * 100) if avg_price > 0 else 0.0  # 수익 계산 및 ZeroDivision 방지
        log_msg = f"[매도] {reason} | 수익률: {profit_pct:.2f}%"   
        
        self.state.update({  # 매도 시 상태 업데이트
            "is_holding": False,
            "avg_buy_price": 0,
            "target_price": 0,
            "stop_loss": 0,
        ...
        await self.save_state()
        level = "WARNING" if event_name == "STOPLOSS" else "INFO"  # stoploss 시 warning 레벨
        await save_log_to_db(
