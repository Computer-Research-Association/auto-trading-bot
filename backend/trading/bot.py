...
class TradingBot:
    ...
    def load_state_sync(self):
        ...
        try:
            with open(self.state_file, 'r', encoding='utf-8') as f:
                ...
                self.last_file_mtime = self.state_file.stat().st_mtime  # 마지막 수정 시간 기록
                return state
        except Exception:
    ...
    async def execute_sell(self, price, reason, event_name="SELL"):
        ...
        self.state.update({
            "is_holding": False, "avg_buy_price": 0, "target_price": 0, "stop_loss": 0,
        ...
        await save_log_to_db(
            ...
            category="TRADE", event_name=event_name,
            message=f"{self.log_prefix} {self.state['last_reason']} | 예정가: {price:,.0f}"
        )
